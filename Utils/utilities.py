...
def get_textscopes(tag):
    ...
    if textscopes == [] or textscopes is None:
        ...
        textscopes = [rdflib.term.URIRef(x.get("REF"))
...
def get_file_dict(script, args, testcase_data, testcases_available):
    ...
    if args.random or args.first or args.last:
        ...
        if args.random:
            if args.random > len(filelist):
                logger.warning("Random file count exceeds file count, defaulting to all files")
                input()
                args.random = len(filelist)
            import random
